...
module.exports = (robot) ->
  ...
  robot.hear /(app ftp) ?(?:@(\w*))?/i, (msg) ->
    user = escape(msg.match[2])
    if user == "undefined"
      msg.send '*Pure-FTPd:* Pure-FTPd is an FTP server. V1.0.36-5 Requires 1 app slot: https://www.appboxes.co/appstore/app/67'
    else
      msg.send '@' + user + ' *Pure-FTPd:* Pure-FTPd is an FTP server. V1.0.36-5 Requires 1 app slot: https://www.appboxes.co/appstore/app/67'
  # ---
  ...
  robot.hear /(app (ubuntu|vnc|ssh)) ?(?:@(\w*))?/i, (msg) ->
    user = escape(msg.match[3])
    if user == "undefined"
      msg.send '*Ubuntu Desktop (VNC):* This install includes VNC, SSH & Various Apps. V18.04-2 Requires 5 app slot: https://www.appboxes.co/appstore/app/97'
    else
      msg.send '@' + user + ' *Ubuntu Desktop (VNC):* This install includes VNC, SSH & Various Apps. V18.04-2 Requires 5 app slot: https://www.appboxes.co/appstore/app/97'
  # ---
